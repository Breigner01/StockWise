...
func (pq *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	...
	if pq.withBrand != nil || pq.withCategory != nil {
		withFKs = true
	}
	if withFKs {
...
func (pq *ProductQuery) loadBrand(ctx context.Context, query *BrandQuery, nodes []*Product, init func(*Product), assign func(*Product, *Brand)) error {
	...
	for i := range nodes {
		if nodes[i].product_brand == nil {
			continue
		}
		fk := *nodes[i].product_brand
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "product_brand" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
