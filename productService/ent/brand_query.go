...
func (bq *BrandQuery) Order(o ...brand.Order) *BrandQuery {
...
func (bq *BrandQuery) Clone() *BrandQuery {
	...
	return &BrandQuery{
		config:      bq.config,
		ctx:         bq.ctx.Clone(),
		order:       append([]brand.Order{}, bq.order...),
...
func (bq *BrandQuery) loadProduct(ctx context.Context, query *ProductQuery, nodes []*Brand, init func(*Brand), assign func(*Brand, *Product)) error {
	...
	for _, n := range neighbors {
		fk := n.product_brand
		if fk == nil {
			return fmt.Errorf(`foreign-key "product_brand" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "product_brand" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
