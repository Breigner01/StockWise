...
func (*Product) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case product.ForeignKeys[0]: // product_brand
			values[i] = new(sql.NullInt64)
		case product.ForeignKeys[1]: // product_category
			values[i] = new(sql.NullInt64)
		default:
	...
	for i := range columns {
		...
		case product.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field product_brand", value)
			} else if value.Valid {
				pr.product_brand = new(int)
				*pr.product_brand = int(value.Int64)
			}
